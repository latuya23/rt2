...
struct DECLSPEC_NV_MATH mat4
{
    ...
    void set_rotx(const nv_scalar & theta);
    void set_roty(const nv_scalar & theta);
    void set_rotz(const nv_scalar & theta);
    void set_rot(const nv_scalar & theta, const vec3 & v);
    void set_rot(const vec3 & u, const vec3 & v);
...
//converts into identity matrix;
extern void identityMatrix(mat4 & M);
