...
void Camera::GenerateRay(Sample sample, Ray* ray){
  ...
  double beta = (double)tan(thetaInRadians) *
                ((m_height*.5) - (double)sample.GetY())/(double)(m_height*.5);
  double alpha = (double)tan(thetaInRadians)*((double)m_width/(double)m_height)*
                 (((double)sample.GetX() - ((double)m_width - 1.0)*.5))/(((double)m_width - 1.0)*.5);
